...
class Graph():
    """ Used for representing graphs. Includes a list of Node objects
        for the graph representation. Holds status for the number
        of cycles in the graph as well as whether it
        is directed or weighted.
    """
    ...
    def _generate_graph(self, available):
        """ From the list of available node values, generate a graph
        by randomly selecting values for nodes and adding [1, 3] neighbors
        for each of the nodes. Assumes that the values in the list
        available are randomly generated.

        :param available: a list of the available values to create
            the graph (domain)
        """
        open_set = []
